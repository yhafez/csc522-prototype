...
import { ai as oe, l as le } from './iframe.734f45ba.js'
import {
	U as wr,
	V as $r,
	_ as ie,
...
import './es.string.from-code-point.cbf2050b.js'
import './es.regexp.flags.cd986f0c.js'
import { j as Q, r as ea, b as kr } from './jsx-runtime.adda36d4.js'
...
function ge(r, a) {
	if (!!r) {
		if (typeof r == 'string') return ta(r, a)
		var e = Object.prototype.toString.call(r).slice(8, -1)
		if ((e === 'Object' && r.constructor && (e = r.constructor.name), e === 'Map' || e === 'Set'))
			return Array.from(r)
		if (e === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(e)) return ta(r, a)
	}
}
...
function fe(r, a) {
	...
	if (e != null) {
		...
		try {
			for (
				e = e.call(r);
				!(l = (d = e.next()).done) && (t.push(d.value), !(a && t.length === a));
				l = !0
			);
		} catch (m) {
			;(c = !0), (i = m)
...
function Dr(r) {
	;(function (a) {
		...
		var i = n(function S(h) {
				return h
					? typeof h == 'string'
						? h
						: typeof h.content == 'string'
						  ? h.content
						  : h.content.map(S).join('')
					: ''
			}, 'stringifyToken'),
			m = n(function S(h) {
						...
						(typeof y != 'string' &&
							(y.type === 'tag' && y.content[0] && y.content[0].type === 'tag'
								? y.content[0].content[0].content === '</'
									? g.length > 0 &&
									  g[g.length - 1].tagName === i(y.content[0].content[1]) &&
									  g.pop()
									: y.content[y.content.length - 1].content === '/>' ||
									  g.push({ tagName: i(y.content[0].content[1]), openedBraces: 0 })
								: g.length > 0 && y.type === 'punctuation' && y.content === '{'
								  ...
								  : g.length > 0 &&
								    ...
								    ? g[g.length - 1].openedBraces--
								    : (N = !0)),
						(N || typeof y == 'string') && g.length > 0 && g[g.length - 1].openedBraces === 0)
					) {
						var E = i(y)
						b < h.length - 1 &&
							(typeof h[b + 1] == 'string' || h[b + 1].type === 'plain-text') &&
							((E += i(h[b + 1])), h.splice(b + 1, 1)),
							b > 0 &&
								(typeof h[b - 1] == 'string' || h[b - 1].type === 'plain-text') &&
								((E = i(h[b - 1]) + E), h.splice(b - 1, 1), b--),
							(h[b] = new a.Token('plain-text', E, null, E))
					}
	...
	;(function (a) {
		...
		;(a.languages.bash = {
			...
			string: [
				...
				{ pattern: /\$'(?:[^'\\]|\\[\s\S])*'/, greedy: !0, inside: { entity: l.entity } },
			],
			environment: { pattern: RegExp('\\$?' + e), alias: 'constant' },
			variable: l.variable,
			function: {
...
function Ir(r) {
	;(function (a) {
		...
		for (
			...
			a.util.type(d) === 'RegExp' && (d = a.languages.javascript[c] = { pattern: d })
			var i = d.inside || {}
				...
				function i(A, k) {
					for (var v = 1, o = l; o < t.length; o++) {
						...
						if (u.type === 'punctuation' && typeof p == 'string') {
							if (A.test(p)) v++
							else if (k.test(p) && (v--, v === 0)) return o
						}
					}
					return -1
				}
				...
				for (n(m, 'addAlias'); l < t.length; ) {
					...
					if (S.type === 'keyword' && S.content === 'mutation') {
						...
						if (
							...
							c(0).content === '{' &&
							(l++, m(c(0), 'property-mutation'), h.length > 0)
						) {
							var y = i(/^\{$/, /^\}$/)
							if (y === -1) continue
							for (var N = l; N < y; N++) {
		...
		Object.defineProperty(r.languages.markup.tag, 'addInlined', {
			value: n(function (e, t) {
				...
				;(l['language-' + t] = {
					...
					(l.cdata = /^<!\[CDATA\[|\]\]>$/i)
				var c = { 'included-cdata': { pattern: /<!\[CDATA\[[\s\S]*?\]\]>/i, inside: l } }
				c['language-' + t] = { pattern: /[\s\S]+/, inside: r.languages[t] }
				var d = {}
				;(d[e] = {
					pattern: RegExp(
...
function _r(r) {
	;(function (a) {
		...
		;(a.languages.markdown = a.languages.extend('markup', {})),
				...
				table: {
					...
					inside: {
						'table-data-rows': {
							...
							lookbehind: !0,
							inside: {
								'table-data': { pattern: RegExp(l), inside: a.languages.markdown },
								punctuation: /\|/,
							},
						},
						'table-line': {
			...
			a.hooks.add('wrap', function (g) {
				if (g.type === 'code-block') {
					...
					var k = a.languages[b]
					if (k) g.content = a.highlight(h(g.content.value), k, b)
		...
		var i = RegExp(a.languages.markup.tag.pattern.source, 'gi'),
			m = { amp: '&', lt: '<', gt: '>', quot: '"' },
			S = String.fromCodePoint || String.fromCharCode
		function h(g) {
			var b = g.replace(i, '')
			return (
				(b = b.replace(/&(\w{1,8}|#x?[\da-f]{1,8});/gi, function (y, N) {
...
function Br(r) {
	;(function (a) {
		var e = /[*&][^\s[\]{},]+/,
			t = /!(?:<[\w\-%#;/?:@&=+$,.!~*'()[\]]+>|(?:[a-zA-Z\d-]*!)?[\w\-%#;/?:@&=+$.~*'()]+)?/,
			l = '(?:' + t.source + '(?:[ 	]+' + e.source + ')?|' + e.source + '(?:[ 	]+' + t.source + ')?)',
			c =
...
function Ue(r, a) {
	...
	if (Object.getOwnPropertySymbols) {
		...
		for (l = 0; l < c.length; l++)
			(t = c[l]),
				!(a.indexOf(t) >= 0) && (!Object.prototype.propertyIsEnumerable.call(r, t) || (e[t] = r[t]))
	}
	return e
}
...
function _e(r) {
	if (Array.isArray(r)) return Lr(r)
}
...
function je(r, a) {
	if (!!r) {
		if (typeof r == 'string') return Lr(r, a)
		var e = Object.prototype.toString.call(r).slice(8, -1)
		if ((e === 'Object' && r.constructor && (e = r.constructor.name), e === 'Map' || e === 'Set'))
			return Array.from(r)
		if (e === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(e)) return Lr(r, a)
	}
}
...
function ar(r) {
	for (var a = 1; a < arguments.length; a++) {
		...
		typeof Object.getOwnPropertySymbols == 'function' &&
			t.push.apply(
				t,
				Object.getOwnPropertySymbols(e).filter(function (l) {
...
function Na(r) {
	...
	if (S) {
		var g = Ze(e, c),
			b
		if (!c) b = ar({}, i, { className: na(i.className) })
		else {
			var y = Object.keys(e).reduce(function (k, v) {
				...
				N = i.className && i.className.includes('token') ? ['token'] : [],
				E =
					i.className &&
					N.concat(
						i.className.filter(function (k) {
							return !y.includes(k)
						}),
					)
			b = ar({}, i, {
		...
		var A = g(a.children)
		return Q(S, { ...b, children: A }, d)
	}
}
n(Na, 'createElement')
var Ye = n(function (a, e) {
...
function Xe(r) {
	var a = r.codeString,
		e = r.codeStyle,
		t = r.containerStyle,
		l = t === void 0 ? { float: 'left', paddingRight: '10px' } : t,
		c = r.numberStyle,
		d = c === void 0 ? {} : c,
		i = r.startingLineNumber
	return Q('code', {
...
function yr(r) {
	var a = r.children,
		e = r.lineNumber,
		t = r.lineNumberStyle,
		l = r.largestLineNumber,
		c = r.showInlineLineNumbers,
		d = r.lineProps,
		i = d === void 0 ? {} : d,
		m = r.className,
		S = m === void 0 ? [] : m,
		h = r.showLineNumbers,
		g = r.wrapLongLines,
		b = typeof i == 'function' ? i(e) : i
	if (((b.className = S), e && c)) {
		var y = $a(t, e, l)
		a.unshift(Ta(e, y))
	}
	return (
		g & h && (b.style = ar({}, b.style, { display: 'flex' })),
		{ type: 'element', tagName: 'span', properties: b, children: a }
	)
}
...
function Da(r) {
	for (
		...
		var l = r[t]
		if (l.type === 'text') e.push(yr({ children: [l], className: Ve(new Set(a)) }))
		else if (l.children) {
...
`,
					...
					if (T === 0) {
						var z = h
								.slice(b + 1, y)
								.concat(yr({ children: [O], className: s.properties.className })),
							H = A(z, I)
						g.push(H)
					} else if (T === x.length - 1) {
						var B = h[y + 1] && h[y + 1].children && h[y + 1].children[0]
						if (B) {
							var K = { type: 'text', value: ''.concat(q) },
								$ = yr({ children: [K], className: s.properties.className })
							h.splice(y + 1, 0, $)
						} else {
							var F = [O],
								R = A(F, I, s.properties.className)
							g.push(R)
						}
					} else {
						var D = [O],
							U = A(D, I, s.properties.className)
						g.push(U)
					}
				}),
					(b = y)
...
function at(r) {
	var a = r.rows,
		...
		t = r.useInlineStyles
	return a.map(function (l, c) {
		return Na({ node: l, stylesheet: e, useInlineStyles: t, key: 'code-segement'.concat(c) })
	})
}
...
function et(r) {
	var a = r.astGenerator,
		e = r.language,
		t = r.code,
		l = r.defaultCodeValue
	if (Ca(a)) {
		var c = Ye(a, e)
		return e === 'text'
			? { value: l, language: 'text' }
			: c
			  ? a.highlight(e, t)
			  : a.highlightAuto(t)
	}
	try {
		return e && e !== 'text' ? { value: a.highlight(t, e) } : { value: l }
	} catch {
		return { value: l }
	}
}
...
function tt(r, a) {
	return n(function (t) {
		var l = t.language,
			...
			g =
				h === void 0
					? {
							...
							style: ar(
								{},
								i['code[class*="language-"]'],
								i['code[class*="language-'.concat(l, '"]')],
							),
					  }
					: h,
			b = t.useInlineStyles,
			...
			N = t.showLineNumbers,
			E = N === void 0 ? !1 : N,
			A = t.showInlineLineNumbers,
			k = A === void 0 ? !0 : A,
			v = t.startingLineNumber,
			o = v === void 0 ? 1 : v,
			u = t.lineNumberContainerStyle,
			p = t.lineNumberStyle,
			s = p === void 0 ? {} : p,
			f = t.wrapLines,
			w = t.wrapLongLines,
			...
			B = H === void 0 ? 'code' : H,
			K = t.code,
			$ = K === void 0 ? (Array.isArray(c) ? c[0] : c) : K,
			F = t.astGenerator,
			R = Ue(t, [
				'language',
				'children',
		...
		var D = E
			...
			or = Ca(F) ? 'hljs' : 'prismjs',
			V = y
				? Object.assign({}, R, { style: Object.assign({}, U, S) })
				: Object.assign({}, R, {
						className: R.className ? ''.concat(or, ' ').concat(R.className) : or,
						style: Object.assign({}, S),
				  })
		if (!F) return kr(z, { ...V, children: [D, Q(B, { ...g, children: $ })] })
		;((f === void 0 && I) || x) && (f = !0), (I = I || at)
		var W = [{ type: 'text', value: $ }],
			M = et({ astGenerator: F, language: l, code: $, defaultCodeValue: W })
		M.language === null && (M.value = W)
		var X = M.value.length + o,
			cr = rt(M, f, T, E, k, o, X, s, x)
		return (
			x
				? (g.style = ar({}, g.style, { whiteSpace: 'pre-wrap' }))
				: (g.style = ar({}, g.style, { whiteSpace: 'pre' })),
			Q(z, {
				...V,
				children: kr(B, {
					...g,
					children: [!k && D, I({ rows: cr, stylesheet: i, useInlineStyles: y })],
				}),
			})
		)
	}, 'SyntaxHighlighter')
...
function lt() {
	for (var r = {}, a = 0; a < arguments.length; a++) {
		var e = arguments[a]
		for (var t in e) ot.call(e, t) && (r[t] = e[t])
	}
	return r
}
n(lt, 'extend')
...
function ft(r) {
	var a = r.space,
		...
		S
	for (m in l)
		(S = new gt(m, c(t, m), l[m], a)),
			e.indexOf(m) !== -1 && (S.mustUseProperty = !0),
			(d[m] = S),
			(i[sa(m)] = m),
			(i[sa(S.attribute)] = m)
	return new dt(d, i, a)
...
var sr = rr,
	...
	Rt = ja,
	L = sr.boolean,
	It = sr.overloadedBoolean,
	vr = sr.booleanish,
	C = sr.number,
	j = sr.spaceSeparated,
	hr = sr.commaSeparated,
	Ot = Ct({
		space: 'html',
		attributes: {
...
function an(r, a) {
	for (var e = r || '', t = a || 'div', l = {}, c = 0, d, i, m; c < e.length; )
		(ca.lastIndex = c),
			(m = ca.exec(e)),
			(d = e.slice(c, m ? m.index : e.length)),
			d &&
				(i
					? i === '#'
						? (l.id = d)
				...
				(c += d.length)),
			m && ((i = m[0]), c++)
	return { type: 'element', tagName: t, properties: l, children: [] }
}
n(an, 'parse$3')
var Zr = {}
Zr.parse = nn
...
function dn(r, a, e) {
	...
	function l(d, i) {
		var m = vn(d, a),
			...
			h = m.tagName.toLowerCase(),
			g
		if (
			((m.tagName = t && pn.call(t, h) ? t[h] : h), i && gn(i, m) && (S.unshift(i), (i = null)), i)
		)
			for (g in i) c(m.properties, g, i[g])
		return (
			Va(m.children, S),
			m.tagName === 'template' &&
				((m.content = { type: 'root', children: m.children }), (m.children = [])),
			m
		)
	}
	function c(d, i, m) {
		...
		m == null ||
			...
			typeof g == 'string' &&
				(S.spaceSeparated
					...
					: S.commaSeparated
					  ? (g = ga(g))
					  : S.commaOrSpaceSeparated && (g = da(ga(g).join(' ')))),
			h === 'style' && typeof m != 'string' && (g = hn(g)),
			h === 'className' && d.className && (g = d.className.concat(g)),
			(d[h] = mn(S, h, g)))
	}
}
n(dn, 'factory$1')
...
function Va(r, a) {
	...
	if (J(a) === 'object' && 'length' in a) {
		for (e = -1, t = a.length; ++e < t; ) Va(r, a[e])
		return
	}
...
function mn(r, a, e) {
	var t, l, c
	if (J(e) !== 'object' || !('length' in e)) return fa(r, a, e)
	for (l = e.length, t = -1, c = []; ++t < l; ) c[t] = fa(r, a, e[t])
	return c
}
...
n(hn, 'style')
function bn(r) {
	for (var a = r.length, e = -1, t = {}, l; ++e < a; ) (l = r[e]), (t[l.toLowerCase()] = l)
	return t
}
...
`,
	...
	jz = {
		...
		DownArrowUpArrow: js,
		DownBreve: Hs,
		DownLeftRightVector: Vs,
		DownLeftTeeVector: Gs,
		DownLeftVector: zs,
		...
		leftrightarrow: jT,
		leftrightarrows: HT,
		leftrightharpoons: VT,
		leftrightsquigarrow: GT,
		leftthreetimes: zT,
		leg: MT,
		leq: PT,
		leqq: ZT,
...
function l3(r, a) {
	var e = {},
		...
		l
	a || (a = {})
	for (l in ya) (t = a[l]), (e[l] = t == null ? ya[l] : t)
	return (
...
function i3(r, a) {
		...
		if ((f === wa && (E = g[N] || 1), (f = r.charCodeAt(y)), f === xa)) {
			...
			for (
				H = y + 1,
					...
					K = Sr[B],
					D--;
				++D < b && ((x = r.charCodeAt(D)), !!K(x));

			)
				(s += lr(x)), B === ir && Yz.call(ha, s) && ((o = s), (O = ha[s]))
			;(p = r.charCodeAt(D) === Qz),
				p && (D++, (u = B === ir ? Pz(s) : !1), u && ((o = s), (O = u))),
				(R = 1 + D - H),
				(!p && !t) ||
					(s
						? B === ir
							? (p && !O
									? q(Ja, 1)
									: (o !== s && ((D = z + o.length), (R = 1 + D - z), (p = !1)),
									  p ||
											((T = o ? Pa : Ya),
											a.attribute
												? ((x = r.charCodeAt(D)),
												  x === a3 ? (q(T, R), (O = null)) : Ma(x) ? (O = null) : q(T, R))
												: q(T, R))),
							  (w = O))
							: (p || q(Za, R),
							  (w = parseInt(s, Kr[B])),
							  s3(w)
									...
									: w in ba
									  ...
									  : ((I = ''),
									    c3(w) && q(Tr, R),
									    w > 65535 &&
												((w -= 65536), (I += lr((w >>> 10) | 55296)), (w = 56320 | (w & 1023))),
									    (w = I + lr(w))))
						: B !== ir && q(Wa, R)),
				w
					? (V(),
					  ...
					  (F = U()),
					  F.offset++,
					  c && c.call(m, w, { start: $, end: F }, r.slice(H - 1, D)),
					  ($ = F))
					: ((s = r.slice(H - 1, D)), (k += s), (E += s.length), (y = D - 1))
		} else f === 10 && (A++, N++, (E = 0)), f === f ? ((k += lr(f)), E++) : V()
	return v.join('')
	function U() {
		return { line: A, column: E, offset: y + (h.offset || 0) }
	}
	function or(W, M) {
		var X = U()
	...
	function V() {
		k && (v.push(k), l && l.call(i, k, { start: $, end: U() }), (k = ''))
	}
}
n(i3, 'parse')
function s3(r) {
	 ...
	 */ var e = (function (t) {
		var l = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i,
			...
			i = {
				...
				util: {
					encode: n(function v(o) {
						return o instanceof m
							? new m(o.type, v(o.content), o.alias)
							: Array.isArray(o)
							  ? o.map(v)
							  : o
										.replace(/&/g, '&amp;')
					...
					clone: n(function v(o, u) {
						u = u || {}
						var p, s
						switch (i.util.type(o)) {
							case 'Object':
								if (((s = i.util.objId(o)), u[s])) return u[s]
								;(p = {}), (u[s] = p)
								for (var f in o) o.hasOwnProperty(f) && (p[f] = v(o[f], u))
								return p
							case 'Array':
								return (
									(s = i.util.objId(o)),
					...
					setLanguage: n(function (o, u) {
						;(o.className = o.className.replace(RegExp(l, 'gi'), '')),
							o.classList.add('language-' + u)
					}, 'setLanguage'),
					currentScript: n(function () {
						...
						try {
							...
							if (o) {
								...
								for (var p in u) if (u[p].src == o) return u[p]
							}
							return null
						}
					}, 'currentScript'),
					isActive: n(function (o, u, p) {
						for (var s = 'no-' + u; o; ) {
							var f = o.classList
							if (f.contains(u)) return !0
							if (f.contains(s)) return !1
							o = o.parentElement
						}
						return !!p
					}, 'isActive'),
				...
				languages: {
					...
					text: d,
					txt: d,
					extend: n(function (o, u) {
						var p = i.util.clone(i.languages[o])
						for (var s in u) p[s] = u[s]
						return p
					}, 'extend'),
					insertBefore: n(function (o, u, p, s) {
						s = s || i.languages
						var f = s[o],
							w = {}
						for (var x in f)
							if (f.hasOwnProperty(x)) {
								if (x == u) for (var q in p) p.hasOwnProperty(q) && (w[q] = p[q])
								p.hasOwnProperty(x) || (w[x] = f[x])
							}
						var T = s[o]
						return (
							(s[o] = w),
							i.languages.DFS(i.languages, function (I, O) {
								O === T && I != o && (this[I] = w)
							}),
							w
						)
					}, 'insertBefore'),
					DFS: n(function v(o, u, p, s) {
						s = s || {}
						var f = i.util.objId
						for (var w in o)
							if (o.hasOwnProperty(w)) {
								...
								var x = o[w],
									q = i.util.type(x)
								q === 'Object' && !s[f(x)]
									? ((s[f(x)] = !0), v(x, u, null, s))
									: q === 'Array' && !s[f(x)] && ((s[f(x)] = !0), v(x, u, w, s))
							}
					}, 'DFS'),
				},
				plugins: {},
				highlightAll: n(function (o, u) {
					i.highlightAllUnder(document, o, u)
				}, 'highlightAll'),
				highlightAllUnder: n(function (o, u, p) {
					var s = {
						callback: p,
						container: o,
						selector:
					...
					for (var f = 0, w; (w = s.elements[f++]); ) i.highlightElement(w, u === !0, s.callback)
				}, 'highlightAllUnder'),
				highlightElement: n(function (o, u, p) {
					var s = i.util.getLanguage(o),
						f = i.languages[s]
					i.util.setLanguage(o, s)
					var w = o.parentElement
					w && w.nodeName.toLowerCase() === 'pre' && i.util.setLanguage(w, s)
					var x = o.textContent,
						q = { element: o, language: s, grammar: f, code: x }
					function T(O) {
						;(q.highlightedCode = O),
							i.hooks.run('before-insert', q),
							(q.element.innerHTML = q.highlightedCode),
							i.hooks.run('after-highlight', q),
							i.hooks.run('complete', q),
							p && p.call(q.element)
					}
					if (
						(n(T, 'insertHighlightedCode'),
						i.hooks.run('before-sanity-check', q),
						(w = q.element.parentElement),
						w &&
							w.nodeName.toLowerCase() === 'pre' &&
							!w.hasAttribute('tabindex') &&
					...
					if (u && t.Worker) {
						...
						;(I.onmessage = function (O) {
							...
							I.postMessage(
								JSON.stringify({ language: q.language, code: q.code, immediateClose: !0 }),
							)
					} else T(i.highlight(q.code, q.grammar, q.language))
				}, 'highlightElement'),
				highlight: n(function (o, u, p) {
					var s = { code: o, grammar: u, language: p }
					...
					return (
						...
						m.stringify(i.util.encode(s.tokens), s.language)
					)
				}, 'highlight'),
				tokenize: n(function (o, u) {
					var p = u.rest
					if (p) {
						for (var s in p) u[s] = p[s]
						delete u.rest
					}
					var f = new g()
				...
				hooks: {
					...
					run: n(function (o, u) {
						var p = i.hooks.all[o]
						if (!(!p || !p.length)) for (var s = 0, f; (f = p[s++]); ) f(u)
					}, 'run'),
				},
				Token: m,
			}
		...
		n(m, 'Token'),
			(m.stringify = n(function v(o, u) {
				...
				var s = {
					...
					f = o.alias
				f && (Array.isArray(f) ? Array.prototype.push.apply(s.classes, f) : s.classes.push(f)),
					i.hooks.run('wrap', s)
				var w = ''
						...
						for (
							...
							if (!(R instanceof m)) {
								var D = 1,
									U
								if (z) {
									if (((U = S(K, F, v, O)), !U || U.index >= v.length)) break
									var M = U.index,
										or = U.index + U[0].length,
										V = F
									for (V += $.value.length; M >= V; ) ($ = $.next), (V += $.value.length)
									if (((V -= $.value.length), (F = V), $.value instanceof m)) continue
									for (
										var W = $;
										W !== o.tail && (V < or || typeof W.value == 'string');
										W = W.next
									)
										D++, (V += W.value.length)
									D--, (R = v.slice(F, V)), (U.index -= F)
								} else if (((U = S(K, 0, R, O)), !U)) continue
								var M = U.index,
									X = U[0],
									cr = R.slice(0, M),
									aa = R.slice(M + X.length),
									qr = F + R.length
		...
		function g() {
			var v = { value: null, prev: null, next: null },
				o = { value: null, prev: v, next: null }
			;(v.next = o), (this.head = v), (this.tail = o), (this.length = 0)
		}
		n(g, 'LinkedList')
		function b(v, o, u) {
			var p = o.next,
				s = { value: u, prev: o, next: p }
			return (o.next = s), (p.prev = s), v.length++, s
		}
		n(b, 'addAfter')
		function y(v, o, u) {
			for (var p = o.next, s = 0; s < u && p !== v.tail; s++) p = p.next
			;(o.next = p), (p.prev = o), (v.length -= s)
		}
		n(y, 'removeRange')
		function N(v) {
			for (var o = [], u = v.head.next; u !== v.tail; ) o.push(u.value), (u = u.next)
			return o
		}
		...
		if ((n(A, 'highlightAutomaticallyCallback'), !i.manual)) {
			...
			k === 'loading' || (k === 'interactive' && E && E.defer)
			...
			: (typeof self > 'u' ? 'undefined' : J(self)) === 'object'
			  ...
			  : (typeof window > 'u' ? 'undefined' : J(window)) === 'object'
			    ...
			    : J(wr) === 'object'
			      ...
			      : {},
	p3 = L3()
dr.Prism = { manual: !0, disableWorkerMessageHandler: !0 }
var d3 = Sn,
	g3 = Zz,
	Qa = Xa.exports,
	f3 = ka,
...
function w3(r, a) {
	...
	if (_.util.type(a) === 'Object') (t = a), (a = null)
	else {
		if (typeof a != 'string') throw new Error('Expected `string` for `name`, got `' + a + '`')
		if (ra.call(_.languages, a)) t = _.languages[a]
...
function S3() {
	var r = _.languages,
		a = [],
		e
	for (e in r) ra.call(r, e) && J(r[e]) === 'object' && a.push(e)
	return a
}
n(S3, 'listLanguages')
function q3(r, a, e) {
	...
	return typeof r == 'string'
		...
		: _.util.type(r) === 'Array'
		  ...
		  : ((t = {
		    ...
		    _.hooks.run('wrap', t),
		    d3(t.tag + '.' + t.classes.join('.'), k3(t.attributes), t.content))
}
n(q3, 'stringify')
function A3(r, a) {
	for (var e = [], t = r.length, l = -1, c; ++l < t; )
		(c = r[l]), c !== '' && c !== null && c !== void 0 && e.push(c)
	for (l = -1, t = e.length; ++l < t; ) (c = e[l]), (e[l] = _.Token.stringify(c, a, e))
	return e
}
n(A3, 'stringifyAll')
function E3(r) {
...
var D3 = xr.div(
	...
	C3 = xr(function (r) {
		var a = r.children,
			e = r.className
		return Q(se, { horizontal: !0, vertical: !0, className: e, children: a })
	...
	z3 = n(function (a) {
		...
		var u = N ? N(b, e) : e.trim(),
			p = ea.exports.useState(!1),
			s = qa(p, 2),
			f = s[0],
			w = s[1],
			x = ea.exports.useCallback(function (q) {
				q.preventDefault()
				var T = Sa.getSelection().toString(),
					I = q.type !== 'click' && T ? T : u
				T3(I)
					.then(function () {
						w(!0),
		...
		return kr(D3, {
			...
			children: [
				...
				d ? Q(ve, { actionItems: [{ title: f ? 'Copied' : 'Copy', onClick: x }] }) : null,
			],
		})
	}, 'SyntaxHighlighter')
export { z3 as SyntaxHighlighter, $3 as createCopyToClipboardFunction, z3 as default }
//# sourceMappingURL=syntaxhighlighter-b07b042a.415e7740.js.map
